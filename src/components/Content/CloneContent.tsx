...
import EditCloneContentDrawer from '../Drawer/EditDrawer/EditCloneContentDrawer';
...
const CloneContent: React.FC<CloneContentProps> = ({ id }) => {
    const [data, setData] = useState<CloneAccount[]>([]);
    const [selectedRow, setSelectedRow] = useState<CloneAccount | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const endpoint = id ? `/clones/group/${id}` : '/clones';
                const response = await _GET(endpoint);
                setData(response);
            } catch (err) {
                console.error('Error fetching clone accounts:', err);
    ...
    const columnHelper = createColumnHelper<CloneAccount>();
